...
function getKickoffWindow(kickoff: unknown) {
  ...
  const day = kickoffDate.toLocaleDateString(undefined, { weekday: "long" });
...
function GameCenter() {
  ...
  const filterItems: Array<{ id: FilterId; label: string; count: number }> = [
    { id: "all", label: "All", count: enhancedGames.length },
  ...
  return (
    <main className="games games-v2">
      <SteelHero
        ...
        rightContent={
          <div className="game-center-hero-panel">
            ...
            <strong>{loading ? "Loading..." : `${games.length} Games`}</strong>
            <small>{liveCount > 0 ? `${liveCount} live now` : "Board active"}</small>
          </div>
        }
      />
      ...
      <section className="game-center-toolbar">
        ...
        {import.meta.env.DEV ? (
          <div className="game-center-dev-tools">
            ...
            <SteelButton onClick={loadTestResults} size="sm" variant="ghost">
              Load Test Results
            </SteelButton>
          </div>
        ) : null}
      ...
      {loading ? (
        <SteelCard className="game-center-state-card" as="section">
          <SteelSectionHeader
            eyebrow="Loading"
      ...
      {error ? (
        <SteelCard className="game-center-state-card is-error" as="section">
          <SteelSectionHeader
      ...
      <div className="games-list game-center-list">
        {Object.entries(groupedGames).map(([groupName, groupGames]) => (
          <section className="game-center-group" key={groupName}>
            <SteelSectionHeader
              eyebrow="NFL Board"
              title={groupName}
            ...
            <div className="game-center-card-grid">
              {groupGames.map(({ game, label, locked, selected, status }) => (
                <SteelCard
                ...
                >
                  <div className="game-center-card-top">
                    <SteelBadge variant={getBadgeVariant(status)}>
                      {getStatusEmoji(game)} {label}
                    </SteelBadge>

                    {status === "live" ? (
                      <span className="game-center-live-ribbon">Live Now</span>
                    ) : null}
                  </div>
