...
function isInvitationRow(
  value: unknown,
): value is InvitationRow {
  if (
    !value ||
    typeof value !== "object" ||
    Array.isArray(value)
...
Deno.serve(async (request: Request) => {
    ...
    return jsonResponse(
      {
        ok: false,
        message:
          "The commissioner session is invalid.",
      },
      401,
      origin,
    );
    ...
    return jsonResponse(
      {
        ok: false,
        message:
          "Unable to read the pending invitation.",
      },
      403,
      origin,
    );
  ...
  if (
    !isInvitationRow(invitationData)
  ) {
    return jsonResponse(
      {
        ok: false,
  ...
  const recordDelivery = async () => {
    const {
      ...
      "record_player_account_invitation_sent",
      {
        p_invitation_id:
          invitationData.id,
      },
    ...
    if (
      ...
      console.error(
        "Unable to record invitation delivery.",
        {
          invitationId:
            invitationData.id,
          error:
            recordError?.message ??
            "No pending invitation was updated.",
        },
      );
    ...
    return jsonResponse(
      {
        ok: false,
        message:
          "Unable to check the Supabase user directory.",
      },
      500,
      origin,
    );
      ...
      return jsonResponse(
        {
          ok: false,
          message:
            "The Supabase account exists, but the invitation status could not be synchronized.",
        },
        500,
        origin,
      );
    ...
    return jsonResponse(
      {
        ...
        message:
          "The existing Supabase account was found and the invitation status was synchronized. The player can use the Cloud Account sign-in form, and the pending invitation will link automatically after sign-in.",
      },
      200,
      origin,
    );
  ...
  const {
    ...
    await adminClient.auth.admin.inviteUserByEmail(
      invitationData.email,
      {
        ...
        data: {
          league_id:
            invitationData.league_id,
          player_id:
            invitationData.player_id,
        },
      },
    );
    ...
    return jsonResponse(
      {
        ok: false,
        message:
          `Supabase could not send the invitation: ${inviteError.message}`,
      },
      502,
      origin,
    );
    ...
    return jsonResponse(
      {
        ok: false,
        message:
          "The email was accepted by Supabase, but the invitation status could not be recorded.",
      },
      500,
      origin,
    );
  ...
  return jsonResponse(
    {
      ...
      status: "sent",
      message:
        "Player invitation email sent successfully.",
    },
    200,
    origin,
  );
