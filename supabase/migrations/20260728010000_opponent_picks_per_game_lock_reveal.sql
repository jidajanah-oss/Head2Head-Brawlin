...
-- Reveal opponent selections only after each individual game's normal
-- database-enforced lock. Both entries must have been submitted at least once,
-- but a later reopen does not hide games that are already permanently locked.
...
begin
  ...
  if not found then
    ...
    if resolved_opponent_player_id is null then
      ...
      resolved_can_reveal :=
        resolved_viewer_submission_status <> 'not-submitted'
        and resolved_opponent_submission_status <> 'not-submitted';
...
comment on function public.get_opponent_pick_reveal(uuid, text, smallint) is
  'Returns only opponent selections for games whose normal five-minute lock has arrived. Open games are omitted from the database response. Both owned head-to-head entries must have been submitted at least once.';
